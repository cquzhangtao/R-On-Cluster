...
public abstract class AbstractBatchRun {
	...
	public void init(){
        installedLibFolder=getAppPath()+File.separator+"installed libs";
	...
	public void runScenarios(){
		...
		for(int i=1;i<scenarios.size();i++){
			...
			String scriptPath=getAppPath()+File.separator+scenarioName+".script.txt";
...
//		return null;
	...
	private  String getValueOfHeaderNoExit(String name,List<String> headers,List<String> values){
	...
	private  String getTemplatePath(List<String> configNames,List<String> configValues){
		String tempPath= getValueOfHeaderNoExit("Template",configNames,configValues);
		if(tempPath!=null&&!tempPath.trim().isEmpty()){
			return tempPath;
		}
		String scenarioName=getScenarioName(configNames,configValues);
		info("No template specified for sceanrio "+scenarioName+", try to use the template.txt under tool folder" );
		return getAppPath()+File.separator+"template.txt";
	}
	
	protected  String getScenarioName(List<String> configNames,List<String> configValues){
		return getValueOfHeader("Name",configNames,configValues);
	}
	protected  boolean isScenarioDisable(List<String> headers,List<String> values){
	...
	protected  void generateScriptForScenario(List<String> names,List<String> values,String scriptPath){
		...
		try {						
			...
			template=".libPaths(\""+installedLibFolder+"\")"+System.lineSeparator()+template;
		...
		for(File file:files){
			...
			if(file.getName().toLowerCase().contains(ext)){
				...
				if(name.lastIndexOf("_")>-1&&name.lastIndexOf("/")+1<name.length()){
					...
					REXP re =run("'"+shortName+"' %in% installed.packages()[,c(1,3)]");
					try {
						if(re!=null && re.asString()!=null &&re.asString().equalsIgnoreCase("true")){
							installed=true;
							continue;
						}
				...
				//if(OSValidator.isWindows()){
					REXP re =run("install.packages(\""+name+"\",\""+installedLibFolder+"\",repos = NULL, type=\"source\")");
					if(re==null){
						error("Cannot install library:"+name);
					}
					
//				}else{
//					String str=rPath+"/R CMD INSTALL --library=\""+installedLibFolder+"\" "+name;
//					try {
//						info(str);
